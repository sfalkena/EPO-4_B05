...
obstacle_x = zeros(1,100);
obstacle_y = zeros(1,100);
    ...
    switch state
     ...
     %       EPOCommunications('transmit','M158')
      ...
      %      EPOCommunications('transmit','M150')
            ...
            % [obstacle, object_x, object_y] =
            % obstacle_Marker_v2(current x, current y , left sensor, right
            % sensor, car orientation)
            % obstacle is 0 when no obstacle, 1 when there is an obstacle
            % sensor_x and sensor_y
            
            %Manually adding 2 obstacles 
         if j==1       [obstacle, object_x, object_y] = obstacle_Marker_v2(150, 250, 50, 50 , pi/2); end
         if j==2       [obstacle, object_x, object_y] = obstacle_Marker_v2(300, 100, 50, 50 , pi/2); end
         %if detected obstacle, plot and store in array 
         if obstacle
             obstacle_x(j) = object_x;
             obstacle_y(j) = object_y;
             scatter(obstacle_x(j), obstacle_y(j), 'm', '*');
         end
            %when obstacle is near future location
             if (abs(xFuture - obstacle_x(j) < 50)) && (abs(yFuture - obstacle_y(j) < 50))
                 %reverse
                             reverse = 1;
                    state = 'reverse';
                    j = j+1;
             end
                                 %reset status that obstacle is detected
                                 obstacle = 0;
                      end
